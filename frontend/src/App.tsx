...
function Navbar({currentPath,onNavigate}){
  ...
  const links=[{path:"/",label:"Home"},{path:"/about",label:"About"},{path:"/classes",label:"Classes"},{path:"/private",label:"Private"},{path:"/clients",label:"Clients"},{path:"/bookings",label:"Bookings"}];
  return(
            ...
            {submitted?(
              ...
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  {[["text","name","Full Name *","Your name"],["email","email","Email *","your@email.com"]].map(([type,name,label,ph])=><div key={name}><label className="block text-xs font-medium text-gray-600 mb-2">{label}</label><input type={type} name={name} value={form[name]} onChange={hc} placeholder={ph} className={ic}/></div>)}
                </div>
        ...
        <div className="max-w-6xl mx-auto">
              ...
              <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-md mx-4">
                ...
                <div className="space-y-4">
                  {[["text","name","Full Name *"],["email","email","Email *"],["tel","phone","Phone"],["number","age","Age"]].map(([type,field,label])=>(
                    <div key={field}><label className="block text-xs font-medium text-gray-500 mb-1">{label}</label><input type={type} value={newClient[field]} onChange={e=>setNewClient({...newClient,[field]:e.target.value})} className={ic}/></div>
                  ))}
          ...
          <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
            <div className="p-6" style={{borderBottom:`1px solid #e8ddd0`}}>
              <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" size={18}/><input type="text" placeholder="Search clients…" value={search} onChange={e=>setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 rounded-xl border border-stone-200 outline-none text-sm"/></div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map(c=>(
                      <tr key={c.id} className="hover:bg-stone-50 transition-colors" style={{borderTop:`1px solid #e8ddd0`}}>
                        ...
                        <td className="px-6 py-4"><div className="flex gap-2">
                          <button onClick={()=>viewClientProfile(c.id)} className="px-3 py-1 text-xs rounded-full bg-purple-50 text-purple-600 hover:bg-purple-100 flex items-center gap-1"><Eye size={12}/>View</button>
                    ...
                    {filtered.length===0&&<tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 text-sm">No clients found</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
                  ...
                  <tbody>
                    {bookings.map(b=>(
                      <tr key={b.id} className="hover:bg-stone-50 transition-colors" style={{borderTop:`1px solid #e8ddd0`}}>
                        ...
                        <td className="px-6 py-4"><span className="px-3 py-1 rounded-full text-xs font-medium capitalize" style={ss[b.status]||ss.confirmed}>{b.status?.toLowerCase()}</span></td>
                        <td className="px-6 py-4"><div className="flex gap-2">
                          ...
                          {b.status?.toUpperCase()!=="CONFIRMED"&&<button onClick={()=>updateStatus(b.id,"confirmed")} className="px-3 py-1 text-xs rounded-full bg-green-50 text-green-600 hover:bg-green-100">Confirm</button>}
                          {b.status?.toUpperCase()!=="CANCELLED"&&<button onClick={()=>updateStatus(b.id,"cancelled")} className="px-3 py-1 text-xs rounded-full bg-yellow-50 text-yellow-600 hover:bg-yellow-100">Cancel</button>}
...
export default function App(){
  ...
  const pages={"/": <Home onNavigate={navigate} onOpenChat={()=>setChatOpen(true)}/>,"/about":<About/>,"/classes":<Classes/>,"/private":<Private/>,"/clients":<Clients/>,"/bookings":<Bookings/>};
  return(
