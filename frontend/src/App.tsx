    ...
    <footer style={{ background: BEIGE_L, borderTop: `1px solid #e8ddd0` }}>
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-3 gap-12">
          ...
          <div><h4 className="font-medium text-gray-800 mb-4 text-sm">Contact</h4><div className="space-y-3">{[[MapPin, "Leeds, UK"], [Mail, "hello@onpilateslane.co.uk"], [Instagram, "@on.pilates.lane"]].map(([Icon, text], i) => <div key={i} className="flex items-start gap-3 text-sm text-gray-600"><Icon size={16} style={{ color: ROSE_D }} /><span>{text}</span></div>)}</div></div>
...
function Home({ onNavigate, onOpenChat }: any) {
  const testimonials = [
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[[Heart, "Expert Instruction", "Led by Bibi Behbehani, bringing precision and passion to every session."], [Users, "Personalised Approach", "Small class sizes ensure individual attention and customised guidance."], [Sparkles, "Premium Experience", "A calming studio space designed for focused, mindful practice in Leeds."]].map(([Icon, title, desc], i) => (
...
function Classes() {
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/bookings`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ client_id: parseInt(clientId), class_id: (bookingClass as any).id, status: "confirmed" }) });
      const data = await res.json();
      ...
      const r = await fetch(`${API}/api/classes/${(bookingClass as any).id}/availability`, { headers: authHeadersNoJson() });
      const newAv = await r.json();
      setAvail(p => ({ ...p, [(bookingClass as any).id]: newAv }));
  ...
  const filteredClasses = selectedStudio ? (classes as any[]).filter(c => c.studio_id === parseInt(selectedStudio)) : [];
  return (
      ...
      {bookingClass && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-md mx-4">
            ...
            <p className="text-sm text-gray-500 mb-5">{(bookingClass as any).name} · {(bookingClass as any).day_of_week} {(bookingClass as any).time}</p>
            <div className="mb-5 px-4 py-3 rounded-2xl text-sm bg-stone-50">
              <div className="font-medium text-gray-700 mb-1">Location</div>
              <div className="text-gray-600 text-xs">{(studios as any[]).find(s => s.id === (bookingClass as any).studio_id)?.name || "Loading..."}</div>
            </div>
            {(avail as any)[(bookingClass as any).id] && <div className="mb-5 px-4 py-3 rounded-2xl text-sm" style={{ background: BEIGE_L }}><span className="font-medium text-gray-700">{(avail as any)[(bookingClass as any).id].available_spots}</span><span className="text-gray-500"> of {(avail as any)[(bookingClass as any).id].capacity} spots left</span>{(avail as any)[(bookingClass as any).id].is_full && <span className="ml-2 text-yellow-600 font-medium">· Waitlist</span>}</div>}
            ...
            <select value={clientId} onChange={e => setClientId(e.target.value)} className={inp}>
              ...
              {(clients as any[]).map(c => <option key={c.id} value={c.id}>{c.name} — {c.email}</option>)}
            </select>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-12 max-w-md mx-auto">
            ...
            <select value={selectedStudio} onChange={e => setSelectedStudio(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-stone-200 outline-none text-sm bg-white">
              ...
              {(studios as any[]).map(s => <option key={s.id} value={s.id}>{s.name} - {s.city}</option>)}
            </select>
          </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8 mb-16">
              {filteredClasses.length === 0 ? (
                ...
                filteredClasses.map((c, i) => {
                  const a = (avail as any)[c.id]; const full = a?.is_full; const lvl = (c.level || "").toLowerCase();
                  const studio = (studios as any[]).find(s => s.id === c.studio_id);
                  return (
                    <div key={i} className="bg-white rounded-3xl p-8 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-4">
                        <div><h3 className="text-2xl font-serif font-light text-gray-800 mb-2">{c.name}</h3><span className="inline-block px-3 py-1 text-xs rounded-full font-medium" style={{ background: (lc as any)[lvl] || "#f3f4f6", color: (lt as any)[lvl] || "#374151" }}>{c.level}</span></div>
...
function Clients() {
  ...
  const fetch_ = async () => { try { setLoading(true); const r = await fetch(`${API}/api/clients`, { headers: authHeadersNoJson() }); if (!r.ok) throw new Error(); setClients(await r.json()); setError(null); } catch { setError("Could not connect to backend." as any); } finally { setLoading(false); } };
  useEffect(() => { fetch_(); }, []);
  const handleSave = async () => {
    if (!newClient.name || !newClient.email) return;
    try {
      const url = editClient ? `${API}/api/clients/${(editClient as any).id}` : `${API}/api/clients`;
  ...
  const viewProfile = async (id: any) => { try { const res = await fetch(`${API}/api/clients/${id}`, { headers: authHeadersNoJson() }); setViewClientDetails(await res.json()); } catch { alert("Failed."); } };
  const filtered = (clients as any[]).filter(c => c.name?.toLowerCase().includes(search.toLowerCase()) || c.email?.toLowerCase().includes(search.toLowerCase()));
          ...
          {viewClientDetails && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
              <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-md mx-4">
                ...
                <div className="space-y-3 text-sm">{[["Client ID", `#${(viewClientDetails as any).id}`], ["Name", (viewClientDetails as any).name], ["Email", (viewClientDetails as any).email], ["Phone", (viewClientDetails as any).phone || "—"], ["Age", (viewClientDetails as any).age || "—"], ["Package", (viewClientDetails as any).package_type || "—"], ["Joined", (viewClientDetails as any).join_date ? new Date((viewClientDetails as any).join_date).toLocaleDateString("en-GB") : "—"]].map(([l, v]) => (<div key={l} className="flex justify-between py-2" style={{ borderBottom: `1px solid #e8ddd0` }}><span className="text-gray-500">{l}:</span><span className="font-medium text-gray-800">{v}</span></div>))}</div>
...
function Bookings() {
  ...
  const fetch_ = async () => { try { setLoading(true); const r = await fetch(`${API}/api/bookings`, { headers: authHeadersNoJson() }); if (!r.ok) throw new Error(); setBookings(await r.json()); setError(null); } catch { setError("Could not connect to backend." as any); } finally { setLoading(false); } };
  useEffect(() => { fetch_(); }, []);
  const updateStatus = async (id: any, status: string) => { try { const b: any = (bookings as any[]).find((b: any) => b.id === id); await fetch(`${API}/api/bookings/${id}`, { method: "PUT", headers: authHeaders(), body: JSON.stringify({ client_id: b.client_id, class_id: b.class_id, status }) }); fetch_(); } catch { alert("Failed."); } };
  const del = async (id: any) => { if (!confirm("Delete this booking?")) return; try { await fetch(`${API}/api/bookings/${id}`, { method: "DELETE", headers: authHeadersNoJson() }); fetch_(); } catch { alert("Failed."); } };
  const viewDetails = async (id: any) => { try { const res = await fetch(`${API}/api/bookings/${id}`, { headers: authHeadersNoJson() }); setViewBooking(await res.json()); } catch { alert("Failed."); } };
  const ss: any = { confirmed: { background: "#d1fae5", color: "#065f46" }, waitlist: { background: "#fef9c3", color: "#713f12" }, cancelled: { background: "#fee2e2", color: "#991b1b" } };
  return (
    <div style={{ background: BEIGE_L }} className="min-h-screen">
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          {viewBooking && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
              <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-md mx-4">
                ...
                <div className="space-y-3 text-sm">{[["Booking ID", `#${(viewBooking as any).id}`], ["Client", (viewBooking as any).client_id], ["Class", (viewBooking as any).class_id], ["Status", (viewBooking as any).status], ["Date", (viewBooking as any).booking_date ? new Date((viewBooking as any).booking_date).toLocaleDateString("en-GB") : "—"]].map(([l, v]) => (<div key={l} className="flex justify-between py-2" style={{ borderBottom: `1px solid #e8ddd0` }}><span className="text-gray-500">{l}:</span><span className="font-medium text-gray-800">{v}</span></div>))}</div>
          ...
          {loading ? <div className="bg-white rounded-3xl p-12 text-center text-gray-400 text-sm shadow-sm">Loading bookings…</div> : error ? <div className="bg-white rounded-3xl p-12 text-center text-red-400 text-sm shadow-sm">{error}</div> : (bookings as any[]).length === 0 ? (
            ...
            <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
              ...
              <div className="p-5 text-center text-xs text-gray-400" style={{ borderTop: `1px solid #e8ddd0` }}>{(bookings as any[]).length} booking{(bookings as any[]).length !== 1 ? "s" : ""}</div>
            </div>
          )}
        </div>
      </section>
...
function formatMsg(text = "") {
  const lines = String(text).split("\n"); const els: any[] = []; let i = 0;
  while (i < lines.length) {
    const line = lines[i];
    if (line.includes("|") && lines[i + 1] && lines[i + 1].includes("---")) {
      const headers = line.split("|").map(h => h.trim()).filter(Boolean); i += 2; const rows: any[] = [];
      while (i < lines.length && lines[i].includes("|")) { rows.push(lines[i].split("|").map(c => c.trim()).filter(Boolean)); i++; }
      els.push(<div key={"t" + i} className="overflow-x-auto my-2 rounded-xl border" style={{ borderColor: "#e8ddd0" }}><table className="w-full text-xs"><thead style={{ background: "#f5f0ea" }}><tr>{headers.map((h, j) => <th key={j} className="px-3 py-2 text-left font-medium text-gray-600">{h.replace(/\*\*/g, "")}</th>)}</tr></thead><tbody>{rows.map((r, j) => <tr key={j} style={{ borderTop: "1px solid #e8ddd0" }}>{r.map((c: string, k: number) => <td key={k} className="px-3 py-2 text-gray-700">{c.replace(/\*\*/g, "")}</td>)}</tr>)}</tbody></table></div>);
      continue;
    }
...
function Chatbot({ forceOpen }: any) {
  const [open, setOpen] = useState(false);
